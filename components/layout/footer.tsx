...
const SERVICE_LINKS = ["Hair care", "Skin care", "Grooming", "Nails care", "Body care"];
const EXPLORE_LINKS = ["Membership", "Locations", "About Carlton", "Careers"];
const BUSINESS_LINKS = ["Franchise Opportunities", "Cluster Partnerships", "Investor Relations"];

const FooterCol = ({ heading, links }: { heading: string; links: string[] }) => (
  <div className="flex flex-col gap-7">
    <span className="text-white font-sans text-[32px]">{heading}</span>
    {links.map((link) => (
      <span key={link} className="text-white/60 font-helvetica text-xl cursor-pointer hover:text-white/90 transition-colors">
        {link}
      </span>
    ))}
  </div>
        ...
        <div className="flex justify-between items-start">
          ...
          <div className="flex flex-col gap-[136px]">
            <div className="relative w-[115px] h-[115px] border border-white/10">
              <Image src={footerLogo} alt="Carlton Footer Logo" fill className="object-cover" />
            </div>
          </div>
